...
describe 'Items Merchant Endpoint' do
  it 'can return the merchant of a given item' do
    ...
    expect(item_merchant[:data].first).to have_key(:type)
    expect(item_merchant[:data].first).to have_key(:attributes)
    expect(item_merchant[:data].first[:attributes][:name]).to eq('Lizard Palace')
  end
end
